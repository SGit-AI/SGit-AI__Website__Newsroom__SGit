...
// Needs the network and Playwright; not part of the standard-library build. Writes assets/shots/<host>/<path>.webp
// (1200x750, top of the page) and assets/shots/manifest.json with the capture time; the build shows a thumbnail
...
const fileFor = (u) => { const x = new URL(u); let p = x.pathname.replace(/\/$/, '/index.html').replace(/^\//, '') || 'index.html'; return path.join(x.host, p.replace(/\.(html|md)$/, '') + '.webp'); };
(async () => {
  ...
  for (const u of [...urls].sort()) {
    ...
    try {
      ...
      await p.screenshot({ path: file, type: 'webp', quality: 60, clip: { x: 0, y: 0, width: 1200, height: 750 } });
