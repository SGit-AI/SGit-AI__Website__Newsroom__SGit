...
**Register id** `developer.guest` · **kind** guest desk · **cadence** when the day's changes give it something to say

## Mission

> Notice code, tools and scripts one project wrote that another could reuse.
...
> This role is failing when: A reuse signal points at code nobody can find from the link, or at something the source calls a proposal.

## Gravity

> Point at the file, not the idea.
...
1. Scan the day's changes for tools, scripts and patterns another project could reuse.
2. Write a signal naming the file on one side and the need on the other.
