<!-- Generated from abp-vault-google-drive-readonly.html by scripts/site/generate.mjs. Edit the page, not this file. -->

# RiskMandate — the behaviour-policy vault for Google Drive, read-only scope

The template Agent Behaviour Policy vault for Google Drive, read-only scope (google/drive/readonly-connector), rendered live from vault vz03p8it: the card, the mandate, the grant with a barrier per row, the delta, the Licence to Operate, the vault’s own app in a sandboxed frame, the file list and the public read key.

Source: https://riskmandate.ai/abp-vault-google-drive-readonly.html
...
The read key travels to the host over a same-page handshake, never in a URL. Nothing you do in either frame touches the vault: it is opened read-only, and the write key is not published. Not loading? [Open it in its own tab ↗](https://dev.vault.sgraph.ai/en-gb/#e04bb066d11479d33ecb9e5d0c1a621e13e4599e8c2b5380001ab16e8c932270:vz03p8it).
