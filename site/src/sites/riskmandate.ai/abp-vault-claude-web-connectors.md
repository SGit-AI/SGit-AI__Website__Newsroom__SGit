<!-- Generated from abp-vault-claude-web-connectors.html by scripts/site/generate.mjs. Edit the page, not this file. -->

# RiskMandate — the behaviour-policy vault for Claude in the browser, connectors on

The template Agent Behaviour Policy vault for Claude in the browser, connectors on (anthropic/claude-web/connectors-on), rendered live from vault wkm5owfl: the card, the mandate, the grant with a barrier per row, the delta, the Licence to Operate, the vault’s own app in a sandboxed frame, the file list and the public read key.

Source: https://riskmandate.ai/abp-vault-claude-web-connectors.html
