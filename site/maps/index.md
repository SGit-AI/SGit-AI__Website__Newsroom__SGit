# Maps

- [How this newsroom runs, drawn](maps/the-newsroom-flow.md)
- [The sgit network as a value chain](maps/sgit-network-wardley.md)
- [RiskMandate.ai's offer as a value chain](maps/riskmandate-wardley.md)
- [The sgit network, dated by its own records](maps/network-timeline.md)
- [The network, as its own links draw it](maps/network.md)
- [The network's recurring concepts, by theme](maps/concepts-mindmap.md)
- [Brief to build, drawn](maps/brief-to-build.md)
