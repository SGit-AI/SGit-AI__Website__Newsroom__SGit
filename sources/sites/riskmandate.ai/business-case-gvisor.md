<!-- Generated from business-case-gvisor.html by scripts/site/generate.mjs. Edit the page, not this file. -->

# RiskMandate — the business case for gVisor

gVisor (Google (open source)), by the risk it changes: the register for a stated agent deployment without it and with it, computed from a public model, from the operator to the board.

Source: https://riskmandate.ai/business-case-gvisor.html

---

# gVisor, by the risk it changes

A sandboxed container runtime. Its networking can be disabled entirely, leaving only a loopback inside the sandbox. Written as an answer, code the agent runs there reaches nothing outside.

**Open source:** Apache-2.0 · Google (open source) · [get involved](https://gvisor.dev/community/)
...
- A sandbox whose external networking can be switched off. “To completely isolate the host and network from the sandbox, external networking can be disabled.” · [gvisor.dev](https://gvisor.dev/docs/user_guide/networking/), read 24 September 2026
...
Its host-network option _decreases the isolation to the host_, so the choice of network mode is part of the case.
...
- Run the agent's code execution in the sandbox, not the agent itself, unless that is intended.
- Choose the network mode deliberately and keep it out of the agent's control.
