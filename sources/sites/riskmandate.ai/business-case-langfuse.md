<!-- Generated from business-case-langfuse.html by scripts/site/generate.mjs. Edit the page, not this file. -->

# RiskMandate — the business case for Langfuse

Langfuse (ClickHouse (company-maintained)), by the risk it changes: the register for a stated agent deployment without it and with it, computed from a public model, from the operator to the board.

Source: https://riskmandate.ai/business-case-langfuse.html

---

# Langfuse, by the risk it changes

Open-source tracing for LLM applications and agents: structured logs of each request, with the prompt, the response and the tools called. Written as an answer, what the agent did can be partly reconstructed, from inside the agent's own code.

**Open source:** MIT, except the ee directories · ClickHouse (company-maintained) · [get involved](https://discord.com/invite/7NXusRtqYU)
...
- Application tracing of prompts, responses and tool steps. “structured logs of every request that capture the exact prompt sent, the model's response, token usage, latency, and any tools or retrieval steps in between.” · [langfuse.com](https://langfuse.com/docs/observability/overview), read 24 September 2026
...
It stores exact prompts and responses, which may include personal data, and the agent holds Langfuse keys. Some self-hosted add-on features require a licence key.
...
- Instrument the agent and its tools, and flush before exit.
- Decide what may be stored, and redact before it is.
- Keep the trace store out of the agent's write reach, or the record is the agent's to edit.

## Published unresolved, for ClickHouse (company-maintained) to settle.
...
- **every request.** The overview speaks of structured logs of every request; the FAQ on missing traces says to flush events manually before the application exits to avoid losing them. [overview](https://langfuse.com/docs/observability/overview) · [missing-traces](https://langfuse.com/faq/all/missing-traces)
